...
contract LiquidityBootstrappingHooksTest is Test, Deployers {
    ...
    function testFullFlowNative() public {
        ...
        // Provide external liquidity
        deal(address(0xBEEF), 1000 ether);
        token1.mint(address(0xBEEF), 1000 ether);
        vm.startPrank(address(0xBEEF));
        token1.approve(address(modifyPositionRouter), 1000 ether);
        modifyPositionRouter.modifyPosition{value: 1000 ether}(key, IPoolManager.ModifyPositionParams(-4000, 0, 10 ether));
        vm.stopPrank();

        // Swap before start time each way
        deal(address(0xdeadbeef), 1000 ether);
        token1.mint(address(0xdeadbeef), 1000 ether);
        vm.startPrank(address(0xdeadbeef));
        token1.approve(address(swapRouter), 1000 ether);
        swapRouter.swap(
        ...
        // zeroForOne = true, swapping NATIVE for token1
